...
- **Calendar vendor: Nylas** (Cal.com in every form, and a from-scratch calendar build, were considered and rejected — see below).
  ...
  - **Nylas** fits: free tier (up to 5 connected calendars, full Scheduler, reminders, webhooks, no credit card), then $15/mo for 10 accounts, $49/mo for 25 — cheaper than Cal.com Teams' flat $12/coach once past a handful of coaches.
    - Requirement #1: use Nylas **"Hosted Authentication" plus whitelabeling** — Revie's own logo and a custom `auth.heyrevie.com` hostname, so the coach never sees a nylas.com URL. **This supersedes an earlier decision to use "Custom Authentication"**, which rested on a misreading of the Nylas docs: Custom Authentication (`POST /v3/connect/custom`) requires the caller to *already hold a refresh token* for the coach's account, so Nylas performs no OAuth handoff at all. Taking that route would mean Revie building and owning a full OAuth integration with Google, Microsoft and Apple separately, including Google's restricted-scope security assessment for calendar access. Both routes require Revie to own its own Google Cloud project and Azure app registration (whitelabeling needs them to register custom callback URIs), so that cost is unavoidable either way — but Hosted plus whitelabel reaches the same "never leaves Revie" experience with far less code.
    ...
    - **Request calendar scopes only, and narrow them deliberately.** Nylas's *default* Google connector asks for `gmail.modify` and `contacts` alongside `calendar`, and its default Microsoft connector asks for `Mail.ReadWrite`, `Mail.Send` and `Contacts.ReadWrite`. None of that is needed for scheduling. Revie's own connectors request roughly `openid`, `userinfo.email`, `userinfo.profile` and `https://www.googleapis.com/auth/calendar`, and nothing more. This matters three ways: a consent screen asking permission to read, send and permanently delete a coach's email will cost sign-ups and deserves to; `gmail.modify` is both restricted *and* high-risk, so including it makes Google's verification materially slower than calendar access alone; and holding email access Revie never uses is a GDPR liability with no corresponding benefit. Scopes are set on the connector and can also be passed per auth request, so keep both narrow.
